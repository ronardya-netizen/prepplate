...
export default function MealCard({ suggestion, isSaved, onToggleSave }: MealCardProps) {
  const { recipe, missingIngredients, pantryIngredients, coveragePct } = suggestion;
  ...
  return (
    <div style={{ background: "#fff", border: coveragePct === 100 ? "1.5px solid #e8470d" : "1px solid #f0e8de", borderRadius: 14, marginBottom: 10, overflow: "hidden", fontFamily: "'Nunito', sans-serif" }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", padding: "12px 14px 8px", gap: 10 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ display: "flex", gap: 5, marginTop: 6, flexWrap: "wrap" }}>
              ...
              {coveragePct === 100 && <Badge variant="sale">Cook now</Badge>}
              {missingIngredients.length > 0 && <Badge variant="missing">Need {missingIngredients.length} item{missingIngredients.length > 1 ? "s" : ""}</Badge>}
            </div>
          </div>
      ...
      <div style={{ display: "flex", alignItems: "center", padding: "7px 14px 10px", borderTop: "1px solid #f5ede5", gap: 4 }}>
        ...
        <span style={{ fontSize: 11, color: "#c09878", marginLeft: 5, fontWeight: 600 }}>
          {coveragePct === 100 ? "✓ All ingredients in pantry" : `${pantryIngredients.length}/${recipe.ingredients.filter(i => !i.isOptional).length} in pantry`}
        </span>
      </div>
    </div>
  );
