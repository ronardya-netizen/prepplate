...
interface MealCardProps { suggestion: SuggestionResult; }

export default function MealCard({ suggestion }: MealCardProps) {
  ...
  useEffect(() => {
    ...
    if (saved) setIsBookmarked(JSON.parse(saved).includes(recipe.id));
  }, [recipe.id]);
  ...
  function toggleBookmark(e: React.MouseEvent) {
    ...
    const updated = current.includes(recipe.id) ? current.filter((id) => id !== recipe.id) : [...current, recipe.id];
      ...
      <Link href={`/meal/${recipe.id}`} style={{ textDecoration: "none", color: "inherit" }}>
        <div style={{ display: "flex", alignItems: "flex-start", padding: "12px 14px 8px", gap: 10 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ fontSize: 12, color: "#c09878", fontWeight: 600, marginTop: 2, lineHeight: 1.4 }}>{recipe.description}</div>
            <div style={{ display: "flex", gap: 5, marginTop: 6, flexWrap: "wrap" }}>
              <Badge variant="neutral">{recipe.prepTimeMin} min</Badge>
              <Badge variant="neutral">~${pricing.totalCost.toFixed(2)}</Badge>
              {pricing.hasSaleItems && <Badge variant="sale">Save ${pricing.totalSavings.toFixed(2)} today</Badge>}
              {missingIngredients.length > 0 && <Badge variant="missing">Buy {missingIngredients.length} item{missingIngredients.length > 1 ? "s" : ""}</Badge>}
            </div>
          </div>
          <button onClick={toggleBookmark} style={{ width: 30, height: 30, borderRadius: "50%", background: isBookmarked ? "#fde8d8" : "#f5f0e8", border: "none", cursor: "pointer", fontSize: 16, display: "flex", alignItems: "center", justifyContent: "center", color: isBookmarked ? "#e8470d" : "#d0c0b0", flexShrink: 0 }} aria-label="Bookmark meal">
            {isBookmarked ? "🔖" : "🏷️"}
          </button>
        </div>
      </Link>
...
function Badge({ children, variant }: { children: React.ReactNode; variant: "neutral" | "sale" | "missing" }) {
  ...
  return <span style={{ fontSize: 10, fontWeight: 700, padding: "3px 8px", borderRadius: 6, fontFamily: "'Nunito', sans-serif", ...styles[variant] }}>{children}</span>;
}