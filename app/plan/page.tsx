...
interface StorePrice { storeName: string; price: number; link: string; }
interface LivePriceResult { name: string; stores: StorePrice[]; }
...
export default function PlanPage() {
  ...
  // Load pantry and saved postal code
  useEffect(() => {
    const id = getUserId();
  ...
  useEffect(() => {
    ...
    if (cleaned.length < 3) { setLocationString(""); setLocationLabel(""); setLocationError(""); return; }
  ...
  // Derive grocery list from pantry
  ...
  const groceryItems = Array.from(allMissingIds).map((id) => {
    const ing = (ingredientsData as IngredientData[]).find((i) => i.id === id);
    return ing ? { id, name: ing.name } : null;
  }).filter(Boolean) as { id: string; name: string }[];

  // Fetch live prices when location + grocery list are ready
  useEffect(() => {
    ...
    if (cached) { setLivePrices(cached); return; }

    setPricesLoading(true);
    setPricesError("");
    fetch(`/api/prices?ingredients=${encodeURIComponent(names.join(","))}&location=${encodeURIComponent(locationString)}`)
      .then((r) => r.json())
      .then((data) => {
  ...
  }, [locationString, allMissingIds.size]);

  function toggleChecked(id: string) {
    setChecked((prev) => { const n = new Set(prev); n.has(id) ? n.delete(id) : n.add(id); return n; });
  }

  // Merge live prices into grocery items
      ...
      <div style={{ background: "#fff", borderRadius: "20px 20px 0 0", marginTop: -8, paddingTop: 16 }}>
        ...
        <div style={{ padding: "0 16px 16px" }}>
          {plannedMeals.length === 0 ? (
            <div style={{ padding: "16px", color: "#c09878", fontSize: 13, fontWeight: 600, textAlign: "center" }}>Loading your meal plan...</div>
              ...
              <div key={meal.id} style={{ padding: "10px 14px", background: "#fff", border: "1px solid #f0e8de", borderRadius: 12, marginBottom: 8 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 11, color: "#c09878", fontWeight: 600, marginTop: 1 }}>Need {missing.length} item{missing.length > 1 ? "s" : ""}</div>
                  </div>
                </div>
              </div>
            );
            ...
            <div style={{ padding: "0 16px" }}>
              {shoppingItems.map((item) => {
                ...
                return (
                  <div key={item.id}>
                    {/* Item row */}
                    <div onClick={() => toggleChecked(item.id)} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", background: checked.has(item.id) ? "#f5f0e8" : "#fff", border: "1px solid #f0e8de", borderRadius: cheapest ? "12px 12px 0 0" : 12, marginBottom: cheapest ? 0 : 6, cursor: "pointer", opacity: checked.has(item.id) ? 0.5 : 1, borderBottom: cheapest ? "none" : undefined }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        {!locationString && <div style={{ fontSize: 11, color: "#c09878", fontWeight: 600 }}>Add postal code to see prices</div>}
                        {locationString && !cheapest && !pricesLoading && <div style={{ fontSize: 11, color: "#c09878", fontWeight: 600 }}>No prices found</div>}
                        {cheapest && <div style={{ fontSize: 11, color: "#2d6a3f", fontWeight: 700, marginTop: 1 }}>Best: {cheapest.storeName} · ${cheapest.price.toFixed(2)}</div>}
                      </div>
                      {cheapest && (
                        <a href={cheapest.link} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} style={{ padding: "6px 10px", borderRadius: 8, background: "#e8470d", color: "#fff", fontSize: 11, fontWeight: 800, textDecoration: "none", flexShrink: 0 }}>
                          Buy
                        </a>
                      )}
                    </div>

                    {/* Store price breakdown — all stores sorted cheapest first */}
                    {cheapest && !checked.has(item.id) && (
                      <div style={{ border: "1px solid #f0e8de", borderTop: "none", borderRadius: "0 0 12px 12px", marginBottom: 6, overflow: "hidden" }}>
                        {item.stores.slice(0, 5).map((store, idx) => (
                          <div key={store.storeName} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "6px 14px", background: idx === 0 ? "#f0faf4" : "#fafafa", borderTop: idx > 0 ? "1px solid #f0e8de" : undefined }}>
                            <span style={{ fontSize: 12, fontWeight: 700, color: idx === 0 ? "#2d6a3f" : "#a08060" }}>
                              {idx === 0 ? "🏆 " : ""}{store.storeName}
                            </span>
                            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                              <span style={{ fontSize: 13, fontWeight: 800, color: idx === 0 ? "#2d6a3f" : "#3a1f0d" }}>${store.price.toFixed(2)}</span>
                              <a href={store.link} target="_blank" rel="noopener noreferrer" style={{ fontSize: 10, fontWeight: 700, color: "#e8470d", textDecoration: "none" }}>→</a>
                            </div>
                          </div>
                        ))}
                      </div>
        ...
        {!locationString && shoppingItems.length === 0 && (
